...
class WktimeController < WkbaseController
	...
	def getissues
		...
		#issues.compact!
		issues = issues.where(status_id: IssueStatus.where(is_closed: false)).select(&:present?)
	...
	def gatherEntries
		...
		unless entryHash.nil?
			entryHash.each_with_index do |entry, i|
				if entry['project_id'].blank? && entry['issue_id'].present?
					entry['project_id'] = Issue.find(entry['issue_id']).project_id rescue ''
				end
	...
	def set_visible_issues(entry)
		...
		if @projectIssues[project_id].blank?
			...
			@projectIssues[project_id] = allIssues.where(status_id: IssueStatus.where(is_closed: false).pluck(:id)).select {|i| i.visible?(@user) }
		end
